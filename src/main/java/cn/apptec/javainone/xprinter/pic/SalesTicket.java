...
public class SalesTicket implements Printable {
    ...
    private String operatorName = "张乐天"; //操作员
    ...
    public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
        ...
        Font font = new Font("楷体", Font.BOLD, 14); //根据指定名称、样式和磅值大小，创建一个新 Font。
        ...
        //设置小票的标题标题
        g2.drawString("陶俊峰娱乐股份有限公司", (float) x + 25, (float) y + heigth);

        line = 2 * (int) heigth; //下一行开始打印的高度
        ...
        // Image src =Toolkit.getDefaultToolkit().getImage("F:\\a"+".png");
        ImageIcon imageIcon = new ImageIcon("F:\\a" + ".jpg");
        // g2.drawImage(imageIcon.getImage(), (int) x, 40, imageIcon.getIconWidth(), imageIcon.getIconHeight(), null);
        //**************************************************************************************//
        ...
        g2.drawLine((int) x, (int) (y + line), (int) x + 210, (int) (y + line));
        line += heigth;
        //设置商品清单

        int contentMarginLeft = 15;
        int contentBodyMarginLeft = 60;
        if (goods != null && goods.size() > 0) {
            for (Orders gdf : goods) {
                g2.drawString("寄件人姓名:", (float) x + contentMarginLeft, (float) y + line);
                g2.drawString(gdf.getSendname(), (float) x + contentBodyMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString("寄件电话:", (float) x + contentMarginLeft, (float) y + line);
                g2.drawString(gdf.getSendtel(), (float) x + contentBodyMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString("寄件地址:", (float) x + contentMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString(gdf.getSendaddress(), (float) x + contentMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString("收件人姓名:", (float) x + contentMarginLeft, (float) y + line);
                g2.drawString(gdf.getRecname(), (float) x + contentBodyMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString("收件人电话:", (float) x + contentMarginLeft, (float) y + line);
                g2.drawString(gdf.getRectel(), (float) x + contentBodyMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString("收件人地址:", (float) x + contentMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString(gdf.getRecaddress(), (float) x + contentMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString("商品类型:", (float) x + contentMarginLeft, (float) y + line);
                g2.drawString(gdf.getGoodstype(), (float) x + contentBodyMarginLeft, (float) y + line);
                line += heigth;
                g2.drawString("配送费:", (float) x + contentMarginLeft, (float) y + line);
                g2.drawString(String.valueOf(gdf.getPrice()), (float) x + contentBodyMarginLeft, (float) y + line);
                line += heigth;
            }

            drawString(g2, "主题", "小说，以刻画人物形象为中心，通过完整的故事情节和环境描写来反映社会生活的文学体裁。人物、情节、环境是小说的三要素。情节一般包括开端、发展、高潮、结局四部分，有的包括序幕、尾声。环境包括自然环境和社会环境。 " +
                    "小说按照篇幅及容量可分为长篇、中篇、短篇和微型小说（小小说）。按照表现的内...", (int) x, (int) y, line, heigth);
        }
    ...
    public void drawString(Graphics2D graphics2D, String title, String value, int x, int y, int height, float heigth) {
        for (String subedString : getSubedStrings(title + ":" + value, 40)) {
            graphics2D.drawString(subedString, (float) x, y + y + line);
            line += heigth;
        }
    }
